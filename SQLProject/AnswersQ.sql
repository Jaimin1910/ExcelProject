...
select count(*) as No_of_invoice, billing_country as Country
from invoice
...
select sum(total) as invoices, billing_city from invoice
...
select customer.customer_id as id, customer.first_name, customer.last_name, sum(invoice.total) as total
from customer
...
join track on invoice_line.invoice_line_id = track.track_id
